#!/usr/bin/env bash
# SessionStart hook — fires on session startup, clear, and compact.
# Injects iron laws, skill auto-load table, and pre-commit gate into session context.
...
# Skip context injection on resume — context is already present.
SOURCE=$(cat | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('source','startup'))" 2>/dev/null || echo "startup")
