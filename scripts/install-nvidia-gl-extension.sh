...
# Determine driver version.  Prefer nvidia-smi when available because it
# reports the exact package version; fall back to parsing glxinfo.
driver=""
...
if [[ -z "$driver" && command -v glxinfo >/dev/null 2>&1 ]]; then
    ...
    driver=$(glxinfo | grep -i "OpenGL version string" | head -n1 \
               | sed -E 's/.*NVIDIA[[:space:]]*([0-9]+\.[0-9]+\.[0-9]+).*/\1/')
fi
...
extension="org.freedesktop.Platform.GL.nvidia-$driver//1.4"
...
flatpak install --user flathub "$extension" -y
