...
### Planned Stories: Real-World Rendering Visual Regression Tests

These stories replace the current synthetic test images with actual rendered particle scenes. They require connecting `FramebufferCapture` (#19) to the visual regression test fixture.

#### Story: Render Single Particle Visual Regression Test

**Type:** Feature | **Size:** M | **Depends On:** #19 (FramebufferCapture)

**As a** developer
**I want to** have a visual regression test that renders a single particle and compares it against a baseline
**So that** I can detect rendering regressions in the most basic particle rendering case

**Acceptance Criteria:**
- [ ] Test creates an OpenGL context (headless via Xvfb)
- [ ] Test renders a single particle at a known position using the project's shader
- [ ] Captures framebuffer to Image using FramebufferCapture
- [ ] Compares captured image against a committed baseline PNG
- [ ] Passes in CI (GitHub Actions, Mesa drivers)
- [ ] Current image always uploaded as artifact

**Files to Create/Modify:**
- `tests/visual-regression/RenderingRegressionTests.cpp` — new test file
- `tests/visual-regression/baselines/single_particle_baseline.png` — baseline image

#### Story: Render Particle Group Visual Regression Test

**Type:** Feature | **Size:** M | **Depends On:** Single Particle story above

**As a** developer
**I want to** have a visual regression test that renders a group of particles and compares against a baseline
**So that** I can detect rendering regressions when multiple particles interact visually (overlap, depth sorting)

**Acceptance Criteria:**
- [ ] Test renders 10-50 particles at known positions
- [ ] Particles have varying colors to verify color rendering pipeline
- [ ] Captures and compares against baseline
- [ ] Uses tolerant comparison (≤2% threshold) to handle Mesa rendering differences
- [ ] Current image always uploaded as artifact

#### Story: Render Particle Cube from Angle Visual Regression Test

**Type:** Feature | **Size:** M | **Depends On:** Particle Group story above

**As a** developer
**I want to** have a visual regression test that renders the initial cube of particles (the default on first load) viewed from an angled camera position
**So that** I can detect regressions in the default scene, camera perspective, and depth rendering

**Acceptance Criteria:**
- [ ] Test creates the default particle cube (as generated on first load)
- [ ] Camera is positioned at an angle to show 3D perspective (not head-on)
- [ ] Captures and compares against baseline
- [ ] Verifies depth rendering (farther particles smaller/dimmer)
- [ ] Current image always uploaded as artifact

**Technical Notes:**
- The "cube" is defined as the initial particle arrangement created when loading for the first time
- Camera should be moved back and angled to see 3D depth
- These tests will be real-world examples — the ones that get uploaded to PR comments
